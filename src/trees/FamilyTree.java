...
public class FamilyTree {
	...
	private static class TreeNode {
		...
		TreeNode getNodeWithName(String targetName) {
			...
			for (TreeNode child : children) {
				...
				TreeNode output = child.getNodeWithName(targetName);
				if (output != null) {
					return output;
				}
			}
	...
	private void addLine(String line) throws TreeException {
		...
		if (colonIndex < 0) {
			throw new TreeException("No Colons");
		}
		...
		*/
		...
		if (root == null) {
			...
			if (parentNode == null) {
				throw new TreeException("Parent not found, something wrong with data file");
			}
		...
		for (String child : childrenArray) {
			TreeNode kid = new TreeNode(child);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException {
		...
		if (node1 == null) {
			...
			throw new TreeException("Node with name1 doesn't exist");
		}
